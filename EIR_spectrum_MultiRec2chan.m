...
saveName = 'DI_VS_DI_beaker_100MHFM18';
saveName = 'test';
...
TriggerTimeout  = 5e-3;                                              % Wait until trigger arrives [ms] (-1: inf)

Setup(DAQ, segCount, trigDelay, inputRange, nSegments, acqMode, detType, TriggerTimeout);                                                                         % Set acquisition, channel and trigger parameters
...
S = zeros(nPts,2);                                                    % Initialize Signal Matrix
SS = zeros(1500,nPts,2);        % 3000 waveforms for 2 channels take 22.4 GB memory
...
%         t_Capture(i)=toc;
        % Fetch the data:
        [S(:,1), tStamp,S(:,2)] = fetchMulti(DAQ, segCount, acqMode, trigDelay,rawData,0,nAvg);
%         t_Fetch(i)=toc;
        for Chan=1:2
            SS(counter,:,Chan) = S(:,Chan);                       % This is only for fast check while acquiring
    ...
    %         toc;    counter
            ...
            ylim(h_axes(Chan),[-120 -90]);
            Title = ['Channel ' num2str(Chan) ':' 'Avg' ' ' num2str(nAvg) ' ' 'Counter' ' ' num2str(counter)];
