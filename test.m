...
acqInfo.SegmentCount = 1024;
acqInfo.Depth = 11008;
acqInfo.SegmentSize = 11008;
...
for i = 1:sysinfo.ChannelCount
    ...
    chan(i).DiffInput  = 0;
    chan(i).InputRange = 2000;    % FSIR 10000, 4000, 2000, 1000, 400, 200              % Set total input voltage range
...
for i = 1:sysinfo.TriggerCount
    ...
    elseif i == 1
        ...
        trig(i).Level = -50;
        trig(i).Source = 1; 
        trig(i).ExtCoupling = CsMl_Translate('AC', 'ExtCoupling');
        trig(i).ExtImpedance = 	CsMl_Translate('50', 'ExtImpedance'); % the external trigger impedance in Ohms. Set 1000000 for HiZ
        trig(i).ExtRange = 10000;
    elseif i == 2
        trig(i).Slope = CsMl_Translate('Positive', 'Slope');
