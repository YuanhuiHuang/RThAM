function [tt_ROI ROI] = show_data_Quick(tt_, R, Slice_T1, Slice_T2, MIP_XYZ, yLim, xLim, SoS, IS_TA, dt, dx, dy, dz, reconParams)
%% show_data
...
if X==1
    Lim = yLim;
    ds = dy;
    figure,imagesc(tt_ROI,Lim,abs(squeeze(ROI(1,:,:))));  grid on; grid minor;
elseif Y==1
    Lim = xLim;
    ds = dx;
    figure,imagesc(tt_ROI,Lim,abs(squeeze(ROI(:,1,:))));  grid on; grid minor;
...
ROI = double(reshape(ROI,X,Y,size(ROI,2)));
