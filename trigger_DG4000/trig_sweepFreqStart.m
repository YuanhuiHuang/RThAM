function Return = trig_sweepFreqStart(trig,nFrequency,nChannel)
%% v1.0 yuanhui 20190209
% Set the start frequency of the sweep and the default unit is "(100) Hz".
...
Command = ['SOURce' num2str(nChannel) ':FREQuency:STARt ' num2str(nFrequency)];
fwrite(trig,Command);

fwrite(trig,['SOURce' num2str(nChannel) ':FREQuency:STARt?']);
Return = fscanf(trig);
disp(['DG4162 ' 'Channel ' num2str(nChannel) ' sweep Start set to (Hz) ' Return]);
