function SIR_Sphere(a,Fs,c,maxPixels)
...
ix=round(length(x)/2);
    ...
    daspect([1 1 1])
    saveas(gcf,sprintf('Images/YZPlane_Fs_%1.2e_c_%d_a_%5.5f.png',Fs,c,a),'png')
    %pause(0.2)
%end
...
iy=round(length(y)/2);
    ...
    daspect([1 1 1])
    saveas(gcf,sprintf('Images/XZPlane_Fs_%1.2e_c_%d_a_%5.5f.png',Fs,c,a),'png')
    %pause(0.2)
%end

% save data
save(sprintf('Images/SensitivityField_Fs_%1.2e_c_%d_a_%5.5f.mat',Fs,c,a),'MaxSig','c','Fs','a','x','y','z')
