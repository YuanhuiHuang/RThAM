...
addpath('C:\Users\Chucheng\Documents\MATLAB\TAM\SAFT recon\FieldII\usefullFunctions')
addpath('C:\Users\Chucheng\Documents\MATLAB\TAM\SAFT recon\FieldII\Field_II_PC7')
addpath('C:\Users\Chucheng\Documents\MATLAB\TAM\SAFT recon\set_plot-v0.8.3\src')
cd C:\Users\Chucheng\Documents\MATLAB\TAM\SAFT recon\FieldII
savefolder = 'C:\Users\Chucheng\Documents\MATLAB\TAM\SAFT recon\FieldII';
...
r = 0.05e-3; % distance from the focal point

for alpha = 0:1:45
    x = 0;
    y = r*sin(alpha*pi/180);
    z = focalRadius + r*cos(alpha*pi/180);
    point = [x y z];
...
%     nShapeConv = conv(nShapeFilt,p1Filt);
    ...
    pause(0.1)
    angle(alpha+1) = alpha;
    sensitivity(alpha+1) = max(nShapeConv)-min(nShapeConv);
end

% figure(111), plot(angle,sensitivity./max(sensitivity))
figure(111), plot(angle,sensitivity)
