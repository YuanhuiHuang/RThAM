function Recon3Dnew(fileParams, reconParams, transducerParams)
    ...
    load([fileParams.data fileParams.dataName]);                           % Load data file for reconstruction
...
%     t_foc   = F_TD/v_s + t_delay;                                          % Delay Time (F/v_s + t_delay) [s]
    ...
    if BPFilter > 0
        ...
        NRows  = 4000;                                                         % Number of signals to be filtered simultaneously
...
%             S((i-1)*NySens + [1:NySens],:) = ZI;
    ...
    subplot(2, 2, 1);
    imagesc(yr,xr,squeeze(max(R,[],1))), daspect([1 1 1]), colorbar, colormap hot
    ...
    ylabel('x (µm)', 'FontWeight', 'bold', 'FontSize', Siz_Ax);
    set(gca, 'units', 'normalized', 'FontSize', Siz_Ti);

    subplot(2, 2, 2);
    imagesc(yr,zr,squeeze(max(R,[],2))), daspect([1 1 1]), colorbar, colormap hot
    xlabel('y (µm)', 'FontWeight', 'bold', 'FontSize', Siz_Ax);
    ylabel('x (µm)', 'FontWeight', 'bold', 'FontSize', Siz_Ax);
    set(gca, 'units', 'normalized', 'FontSize', Siz_Ti);
