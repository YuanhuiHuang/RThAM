function ezshow_rsom_Quick_2018(Ratio, data_Name)
%%
...
Slice_T1 = (8.*1e-6).*Ratio; 
Slice_T2 = (9.*1e-6).*Ratio; 
...
Slice_T1_Start=trigDelay.*2e-9;  Slice_T1_End=10.5e-6;  % WOiLine
...
if ~IS_TA    % title('US B scan');   
    ...
    tDelay = tDelay/2;
    if tDelay==0 tDelay = Slice_T1./2; end
    ind = [1:1:size(SS,2)]';
...
%         R((R<=0.2.*min(R(:)))) = 0; 
%         show_data_Quick;
        [tt_ROI ROI] = show_data_Quick_2018(tt_, R, Slice_T1, Slice_T2, MIP_XYZ, yLim, xLim, SoS, IS_TA, dt, dx, dy, dz, reconParams);
