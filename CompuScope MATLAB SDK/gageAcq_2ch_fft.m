...
function [sAvg] = gageAcq(handle,rawdata, nAvg)
...
if rawdata
    ...
    sAvg = zeros(2, nPts, 'int16');
else
    s    = zeros(1, nPts, 'double');
    sAvg = zeros(2, nPts, 'double');
end

for ii = 1:ChannelSkip:sysinfo.ChannelCount
    

    ret = CsMl_Capture(handle);     %the acquisition is initiated    % Start acquisition and await trigger event
